...
class IsaacRandom(random.Random):
    ...
    def seed(self, seed=None):
        ...
        def mix():
            init_state[0] ^= ((init_state[1] << 11) & INT_MASK)
            init_state[3] += init_state[0]
            init_state[3] &= INT_MASK
            init_state[1] += init_state[2]
            init_state[1] &= INT_MASK
            init_state[1] ^= (init_state[2] >> 2)
            init_state[4] += init_state[1]
            init_state[4] &= INT_MASK
            init_state[2] += init_state[3]
            init_state[2] &= INT_MASK
            init_state[2] ^= ((init_state[3] << 8) & INT_MASK)
            init_state[5] += init_state[2]
            init_state[5] &= INT_MASK
            init_state[3] += init_state[4]
            init_state[3] &= INT_MASK
            init_state[3] ^= (init_state[4] >> 16)
            init_state[6] += init_state[3]
            init_state[6] &= INT_MASK
            init_state[4] += init_state[5]
            init_state[4] &= INT_MASK
            init_state[4] ^= ((init_state[5] << 10) & INT_MASK)
            init_state[7] += init_state[4]
            init_state[7] &= INT_MASK
            init_state[5] += init_state[6]
            init_state[5] &= INT_MASK
            init_state[5] ^= (init_state[6] >> 4)
            init_state[0] += init_state[5]
            init_state[0] &= INT_MASK
            init_state[6] += init_state[7]
            init_state[6] &= INT_MASK
            init_state[6] ^= ((init_state[7] << 8) & INT_MASK)
            init_state[1] += init_state[6]
            init_state[1] &= INT_MASK
            init_state[7] += init_state[0]
            init_state[7] &= INT_MASK
            init_state[7] ^= (init_state[0] >> 9)
            init_state[2] += init_state[7]
            init_state[2] &= INT_MASK
